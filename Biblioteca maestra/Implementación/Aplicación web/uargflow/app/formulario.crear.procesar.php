 ...
 * en la base de datos.
 */
BDConexion::getInstancia("bdFormularios")->query("" .
...
if (empty($formulario->getFechaApertura()) && empty($formulario->getFechaCierre())) {
    $consulta = BDConexion::getInstancia()->query("" .
            "INSERT INTO `formulario`(`idCreador`, `emailReceptor`, `titulo`, `descripcion`, `fechaCreacion`, `fechaApertura`, `fechaCierre`, `estaHabilitado`, `cantidadRespuestas`) " .
...
if ($consulta) { // Si la inserción del formulario se completó exitosamente, se continúa con el procesamiento.
    ...
    foreach ($formulario->getCampos() as $campo) {
        $consulta = BDConexion::getInstancia()->query("" .
                "INSERT INTO `campo`(`idFormulario`, `titulo`, `descripcion`, `esObligatorio`, `posicion`) " .
